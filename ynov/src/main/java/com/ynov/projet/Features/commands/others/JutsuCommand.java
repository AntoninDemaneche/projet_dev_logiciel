                            ...
                            if (sender instanceof Player && sender.isOp()) {
                                ...
                                if (skill != null) {
                                    ...
                                    if (pInfo.getSkills().containsKey(skill)) {
                                        ...
                                        pInfo.getPlayer().sendMessage(ChatColor.RED + "Vous ne connaissez pas cette technique !");
                                    }
                                } else {
                                    sender.sendMessage(ChatColor.RED + "La technique " + split[1] + " n'existe pas !");
                                }
                            } else {
                                sender.sendMessage(ChatColor.RED + "Vous n'avez pas la permission d'utiliser cette commande !");
                            }
                       ...
                       /* if (split.length == 2) {
                            if (sender instanceof Player && sender.isOp()) {
                                ...
                                if (skill != null) {
                                    PlayerInfo pInfo = PlayerInfo.getPlayerInfo(((Player) sender));
                                    if (!pInfo.getSkills().containsKey(skill)) {
                                        if (skill.isNeedMastery()) {
                                            pInfo.updateSkill(skill, SkillMastery.UNLEARNED);
                                        } else {
                                            pInfo.updateSkill(skill, SkillMastery.LEARNED);
                                        }
                                    } else {
                                        pInfo.getPlayer().sendMessage(ChatColor.RED + "Vous connaissez déjà cette technique !");
                                    }
                                } else {
                                    sender.sendMessage(ChatColor.RED + "La technique " + split[1] + " n'existe pas !");
                                }
                            ...
                            if (sender.isOp()) {
                                ...
                                if (skill != null) {
                                    ...
                                    if (target != null) {
                                        ...
                                        if (!tInfo.getSkills().containsKey(skill)) {
                                            if (skill.isNeedMastery()) {
                                                tInfo.updateSkill(skill, SkillMastery.UNLEARNED);
                                            } else {
                                                tInfo.updateSkill(skill, SkillMastery.LEARNED);
                                            }
                                            sender.sendMessage(target.getDisplayName() + ChatColor.GREEN + " a appris la technique: " + ChatColor.GOLD + skill.getName());
                                        } else {
