        ...
        if (split.length == 2) {
            if (sender.getServer().getPlayer(split[1]) != null) {
                ...
                if (StringUtils.isNumeric(split[0])) {
                    ...
                    sender.sendMessage(ChatColor.BLUE + "Tu envoie " + ChatColor.GOLD + amout + ChatColor.BLUE + " de chakra à " + target.getDisplayName() + ChatColor.BLUE + " !");
                    targetInfo.addMana(amout);
                    target.sendMessage(ChatColor.BLUE + "Tu reçois " + ChatColor.GOLD + amout + ChatColor.BLUE + " de chakra de la part de " + ((Player) sender).getDisplayName() + ChatColor.BLUE + " !");
