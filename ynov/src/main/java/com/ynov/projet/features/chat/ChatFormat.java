...
public class ChatFormat extends Feature
{
    ...
    private void initialize()
    {
        ...
        addRule("?#", "{range:3}{restricted:mj, color:AQUA}** %m");
        addRule("#?", "{range:3}{restricted:mj, color:AQUA}** %m");
        addRule("-?","{range:7}{restricted:mj, color:AQUA}** %m");
        addRule("?-","{range:7}{restricted:mj, color:AQUA}** %m");
        addRule("?", "{range:20}{restricted:mj, color:AQUA}** %m");
        addRule("?+", "{range:50}{restricted:mj, color:AQUA}** %m");
        addRule("+?", "{range:50}{restricted:mj, color:AQUA}** %m");
        addRule("?!", "{range:100}{restricted:mj, color:AQUA}** %m");
        addRule("!?", "{range:100}{restricted:mj, color:AQUA}** %m");
        addRule("?:", "{range:-1, restricted:mj, color:AQUA, foreveryworld:true}%m");
        addRule(":?", "{range:-1, restricted:mj, color:AQUA, foreveryworld:true}%m");
        ...
        addRule("@", "{range:-1, restricted:mj, foreveryworld:true, onlyfor:mj, addtarget:true}{color:LIGHT_PURPLE,commandonclick:$}<%a->@%t> %m");
                ...
                switch(readState)
                {
                    ...
                    case PARAMETER:
                        ...
                        if("color".equals(context.getCurrentAttribute()))
                        {
                            try
                            {
                                context.setChatColor(ChatColor.valueOf(parameter));
                            }
                            catch(Exception e)
                            {
    ...
    public static boolean HasReallyAnAnimal(Player p) {
        ...
        if(item.getType() != Material.AIR) {
            ...
            if(tag.hasKey("seisan") && tag.getString("seisan").contains("animal")) {
                // Blabla vérif du tag
                hasAnAnimal = true;
            }
        }
        if(!hasAnAnimal && item2.getType() != Material.AIR) {
            net.minecraft.server.v1_15_R1.ItemStack stack = CraftItemStack.asNMSCopy(item2);
            NBTTagCompound tag = stack.getTag() != null ? stack.getTag() : new NBTTagCompound();
            if(tag.hasKey("seisan") && tag.getString("seisan").contains("animal")) {
                // Blabla vérif du tag
                hasAnAnimal = true;
